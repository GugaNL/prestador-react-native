...
//Temporário
import { historyServices } from '../../Util/Mocks/MockServicesProvided'

//Actions
import { changeDetailProvidedService } from '../../Store/Actions/ActionServicesProvided'
...
class ServicesProvided extends Component {
    ...
    openDetails(item) {
        ...
        this.props.changeDetailProvidedService(item)
        this.props.navigation.navigate('DetailServicesProvided')
    }
    ...
    searchFilter = (text) => {
        ...
        let newData = listSearch.filter(item => {
            const itemData = `${item.descripton.toLowerCase()} ${item.customer.toLowerCase()}`
    ...
    render() {
        ...
        return (
            <View style={{ flex: 1, backgroundColor: '#FBFBFB' }}>
                <HeaderMain title='Serviços prestados' backButton={true} pressBack={() => this.props.navigation.goBack()} />
                <Search searchTerm={this.state.searchTerm} placeholder='Serviço ou cliente' callParentSearch={this.searchFilter.bind()} />
                            ...
                            <View style={{ flex: 1, flexBasis: 0, marginBottom: 20, marginTop: 15, marginLeft: 15, marginRight: 15, backgroundColor: projectColors.white, marginBottom: 20, borderRadius: 5, paddingTop: 10, paddingBottom: 10, elevation: 5, borderTopWidth: 2, borderTopColor: item.status == 'finalizado' ? projectColors.mainGreen : projectColors.mainYellow }}>
                                <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center' }} onPress={() => this.openDetails(item)}>
                                    ...
                                    <View style={{ alignItems: 'flex-start', width: '75%' }}>
                                        <Text style={{ marginTop: 2, marginLeft: 5, fontFamily: 'Roboto', fontSize: 16, fontWeight: 'bold', color: projectColors.lightBlack }}>{item.description}</Text>
                                        <View style={{ flexDirection: 'row' }}>
                                            <View style={{ width: '80%' }}>
                                                <Text style={{ marginTop: 5, marginLeft: 5, fontFamily: 'Roboto', color: '#d3d3d3' }}>{item.customer}</Text>
...
const mapDispatchToProps = dispatch => (
    {
        changeDetailProvidedService: values => dispatch(changeDetailProvidedService(values))
    }
)
